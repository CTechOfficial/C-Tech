...
//n'est pas pris en compte dans MacroAccueil : impacte toute la fin de MacroAccueil
function getSelectIndicateur($select_indicateur){
		$indicateur = ORM::for_table('select_indicateur')->select('LIB_IND')->where('CODE',$select_indicateur)->find_array();
		$indicateur = explode(':',$indicateur);
		$indicateur = explode('"',$indicateur[1]);
		return $indicateur[1];
	}
//n'est pas pris en compte dans MacroAccueil
function getSelectFamProd($select_famille_produit){
		$famille_produit = ORM::for_table('select_famille_produit')->select('LIB_FAMILLE')->where('CODE',$select_famille_produit)->find_array();
		$famille_produit = explode(':',$famille_produit);
		$famille_produit = explode('"',$famille_produit[1]);
		return $famille_produit[1];
	}

function getSelectCumul($code_select_cumul){
	if($code_select_cumul == 0){
		return 'data_1';
	}else if($code_select_cumul == 1){
		return 'data_0';
	}
}
function getTableauAcceuilDM($token,$code_select_temps, $code_select_cumul, $code_select_indicateur ){
	...
	//$code_select_indicateur 0 || 1 || 2 || 3
	$app = App::getInstance();
	if($token === $app->getCookie('USER_TOKEN')){
		//Recupération zone_geographique
		$select_ville_for_zone_geo = json_encode(ORM::for_table(getSelectCumul($code_select_cumul))->select('VILLE')->where('ID_PROFIL',getProfilID($token))->find_array());
		...
		$id_magasin = json_encode(ORM::for_table('securite')->select('id_magasin')->where('id_profil',getProfilID($token))->find_array());
